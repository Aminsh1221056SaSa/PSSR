...
JSGantt.TaskItem = function (pID, pName, pStart, pEnd, pColor, pLink, pMile, pRes, pComp, pGroup, pParent, pOpen, pwbsCode, pProgress,pActivityCount, pDepend, pCaption) {
    ...
    var vDuration = '';
    var vwbsCode = pwbsCode;
    var vProgress = pProgress;
    var vActivityCount = pActivityCount;
    var vLevel = 0;
    var vNumKid = 0;
    var vVisible = 1;
    var x1, y1, x2, y2;
    ...
    this.getwbsCode = function () { return vwbsCode };
    this.getActivityCounter = function () { return vActivityCount };
    this.getProgress = function () { return vProgress };
    ...
    this.setGroup = function (g) { vGroup = g; };
...
JSGantt.GanttChart = function (pGanttVar, pDiv, pFormat) {
    ...
    this.setShowInput = function (pShow) { vShowInput = pShow; };
    this.setShowActivityCount = function (pShow) { vShowActivityCount = pShow; };

    this.setShowRes = function (pShow) { vShowRes = pShow; };
    this.setShowProgress = function (pShow) { vShowProgress = pShow; };
    ...
    this.getShowProgress = function () { return vShowProgress };
    this.getShowInput = function () { return vShowInput };
    this.getShowActivityCount = function () { return vShowActivityCount };
    ...
    this.HidenRightside = function () {
        var parent = document.getElementById('rightside');
        parent.style.display = 'none';
    };


    this.Draw = function (hname1, hname2, hname3, hname4, hname5, hname6, hname7, hname8) {
        ...
        if (vTaskList.length > 0) {
            ...
            vMainTable =
                ...
                '<TD vAlign=top bgColor=#ffffff id=theTable-firsttd>';
            ...
            vLeftTable =
                '<DIV class=scroll id=leftside style="width:' + vLeftWidth + 'px"><TABLE id=theTable-second cellSpacing=0 cellPadding=0 border=0><THEAD>';
            ...
            vLeftTable +=
                ...
                '  <TH style="BORDER-TOP: #efefef 1px solid; WIDTH: 1px; HEIGHT: 20px">.</TH>' +
            '  <TH style="BORDER-TOP: #efefef 1px solid; WIDTH: 100px; HEIGHT: 20px;text-align:center"><NOBR>' + hname1 + '</NOBR></TH>';

            if (vShowRes == 1) vLeftTable += '  <TH style="text-align:center;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: ' + vNameWidth + 'px; HEIGHT: 20px" align=center nowrap>' + hname7 + '</TH>';
            if (vShowDur == 1) vLeftTable += '  <TH style="text-align:center;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>' + hname2 + '</TH>';
            if (vShowActivityCount == 1) vLeftTable += '  <TH style="text-align:center;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>' + hname8 + '</TH>';

            if (vShowwbsCode == 1) vLeftTable += '  <TH style="text-align:center;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>' + hname6 + '</TH>';
            if (vShowComp == 1) vLeftTable += '  <TH style="text-align:center;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>' + hname3 + '</TH>';
            if (vShowProgress == 1) vLeftTable += '  <TH style="text-align:center;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>' + hname7 + '</TH>';
            if (vShowStartDate == 1) vLeftTable += '  <TH style="text-align:center;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>' + hname4 + '</TH>';
            if (vShowEndDate == 1) vLeftTable += '  <TH style="text-align:center;BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>' + hname5 + '</TH>';
            ...
            for (i = 0; i < vTaskList.length; i++) {
                ...
                vLeftTable +='<TD id="td_' + vID + '" class=gname style="WIDTH: 60px; HEIGHT: 20px; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px;" nowrap>';
                   
                var jjkk = '<NOBR><span style="color: #aaaaaa">';
                for (j = 1; j < vTaskList[i].getLevel(); j++) {
                    jjkk += '&nbsp&nbsp&nbsp&nbsp';
                }
                ...
                if (vTaskList[i].getGroup()) {
                    ...
                    var dp = vTaskList[i].getMile();
                    if (vTaskList[i].getOpen() == 1) {
                        jjkk += '<SPAN  id="group_' + vID + '" style="color:#000000;cursor:pointer; font-weight:bold; FONT-SIZE: 12px;" onclick="JSGantt.folder(' + vID + ',' + vGanttVar + ');' + vGanttVar + '.DrawDependencies();">&ndash;</span><span style="color:#000000">&nbsp</SPAN>';
                    }
                    else {
                        jjkk += '<SPAN id="group_' + vID + '" style="color:#000000; cursor:pointer;font-weight:bold; FONT-SIZE: 12px;" onclick="JSGantt.folder(' + vID + ',' + vGanttVar + ');' + vGanttVar + '.DrawDependencies();">+</span><span style="color:#000000">&nbsp</SPAN>';
                    }
                } else {
                    var lastTag = '<span class="lastwbsChild" style="color: #000000; cursor:pointer;font-weight:bold; FONT-SIZE: 12px;"></span>';
                    jjkk += lastTag;
                }

                jjkk += '<span> ' + vTaskList[i].getName() + '</span></NOBR></TD>';
                ...
                if (vShowActivityCount == 1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 20px; TEXT-ALIGN: center; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR class="btn btn-default" style=cursor:pointer;width:60px onclick=JSGantt.taskLink("' + vTaskList[i].getLink() + '","' + vTaskList[i].getID() + '",200);>' + vTaskList[i].getActivityCounter() + '</NOBR></TD>';
                if (vShowwbsCode == 1) vLeftTable += '  <TD class=gname style="WIDTH: ' + vNameWidth + 'px; HEIGHT: 20px; TEXT-ALIGN: center; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + vTaskList[i].getwbsCode() + '</NOBR></TD>';


                if (vShowComp == 1)
                {
                    if (vShowInput == 1) {
                        vLeftTable += '<TD class=gname style="WIDTH: 60px; HEIGHT: 20px; TEXT-ALIGN: center; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR><input class=wbs-input-wf type=text id=wbs-wf-' + vTaskList[i].getID() + ' value=' + vTaskList[i].getCompStr() + '></input></NOBR></TD>';
                    }
                    else {
                        vLeftTable += '<TD class=gname style="WIDTH: 60px; HEIGHT: 20px; TEXT-ALIGN: center; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR><p>' + vTaskList[i].getCompStr() + '</p></NOBR></TD>';
                    }
                }

                if (vShowProgress == 1) {
                    var progress = vTaskList[i].getProgress();
                    var sn1 = 'bg-red';
                    if (progress <= 0) {
                        sn1 = 'bg-red';
                    }
                    ...
                    vLeftTable += '<TD class=gname style="WIDTH: 60px; HEIGHT: 20px; TEXT-ALIGN: center; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR><p class=' + sn1 + ' >' + progress + '</p></NOBR></TD>';
                }
            ...
            vLeftTable += '</TD></TR>' +
                '<TR><TD id=jsgantt-bottom border=1 colspan=5 align=left style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 11px; BORDER-LEFT: #efefef 1px solid; height=18px">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Format:';
            ...
            for (i = 0; i < vTaskList.length; i++) {
                ...
                if (vTaskList[i].getMile()) {
                    ...
                    '  <div id=taskbar_' + vID + ' title="' + vTaskList[i].getName() + ': ' + vDateRowStr + '" style="height: 16px; width:12px; overflow:hidden; cursor: pointer;" onclick=JSGantt.taskLink("' + vTaskList[i].getLink() + '","' + vTaskList[i].getID() + '",200);>';
                    ...
                    if (vTaskList[i].getGroup()) {
                        ...
                        'cursor: pointer;" onclick=JSGantt.taskLink("' + vTaskList[i].getLink() + '","' + vTaskList[i].getID() + '",200);>' +
                            '</div>' +
                            '</div>' +
                        ...
                        vRightTable +=
                            ...
                            'onclick=JSGantt.taskLink("' + vTaskList[i].getLink() + '","' + vTaskList[i].getID() + '",200); >' +
    ...
    for (i = 0; i < vList.length; i++) {
        if (vList[i].getID() == pID) {
            if (vList[i].getOpen() == 1) {
                ...
                if (JSGantt.isIE())
                    JSGantt.findObj('group_' + pID).innerText = '_';
                else
                ...
                if (JSGantt.isIE())
                    ...
                    JSGantt.findObj('group_' + pID).textContent = '_';
            }
    ...
    for (var i = 0; i < vList.length; i++) {
        if (vList[i].getParent() == pID) {
            ...
            if (pTop == 1) {
                ...
                if (JSGantt.isIE()) { // IE;
                    if (JSGantt.findObj('group_' + pID).innerText == '_') {
                        JSGantt.findObj('child_' + vID).style.display = "";
                        JSGantt.findObj('childgrid_' + vID).style.display = "";
                        vList[i].setVisible(1);
                    }
                    ...
                    if (JSGantt.findObj('group_' + pID).textContent == '_') {
                        JSGantt.findObj('child_' + vID).style.display = "";
                        JSGantt.findObj('childgrid_' + vID).style.display = "";
                        vList[i].setVisible(1);
                    }
